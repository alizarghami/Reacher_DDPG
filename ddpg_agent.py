...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        new_state_value_target = self.critic_target.forward(next_states, action_target)
        state_value_local = self.critic_local.forward(states, actions)
        ...
        # Compute loss
        actor_loss = torch.mean(-1 * self.critic_local.forward(states, action_local))
        # Minimize the loss
    ...
    def save_model(self):
        ...
        torch.save(actor_state_dict, 'models/actor_checkpoint.pth')
        torch.save(critic_state_dict, 'models/critic_checkpoint.pth')
        
    def load_model(self):
        actor_state_dict = torch.load('models/actor_checkpoint.pth')
        critic_state_dict = torch.load('models/critic_checkpoint.pth')
